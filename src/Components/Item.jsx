...
const Item = ({ color, name, dataFromServer }) => {
  const [data, setData] = useState(
    checkData(dataFromServer) ? dataFromServer : []
  )
  const [error, setError] = useState(false)
  const [loading, setLoading] = useState(
    checkData(dataFromServer) ? false : true
  )
    ...
    <>
      ...
      {!loading && !error && checkData(data) && (
        <div className="flex flex-col justify-end items-start">
          ...
          <div
            className={`flex flex-col w-full gap-4 rounded-lg rounded-tl-none p-2 lg:p-6 from-slate-200 ${viaColor} to-slate-200 from-10% via-35% to-100%  xl:from-20% xl:via-50% xl:to-80% bg-gradient-to-bl`}
          >
            <div
              className={`w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 justify-center items-center `}
            >
              {data
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="p-2 bg-blue-600 hover:bg-green-600 text-white rounded active:animate-ping select-none"
                        >
                          Add To Cart
                        </button>
                      </div>
            ...
            {checkData(data) && data?.length > 4 && (
              <div className="w-full flex justify-end">
                <Link
                  to={`/category/${name}`}
                  className="bg-white/40 p-1 rounded"
                >
                  More...
                </Link>
              </div>
