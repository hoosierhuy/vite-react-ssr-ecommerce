...
const CartBox = () => {
  ...
  useEffect(() => {
    ...
    if (!cart.length && parsStorage) {
      ...
      if (itemPrices.length) setPrices([...itemPrices])
    }
  }, [])
  const showCartBox = () => {
  ...
  return (
    <div
      id="toggleCart"
      className="flex select-none p-2 items-center gap-1 bg-white relative rounded-lg"
      onMouseOver={showCartBox}
    ...
    >
      <div className="hidden lg:block">
        <Link to={`/cart`} className="flex gap-1 items-center">
          ...
          {cart?.length > 0 && (
            <div className="text-xs bg-red-600 rounded-full text-white py-0 px-1">
              {cart?.length}
            </div>
          )}
        </Link>
      ...
      <div
        id="cartBox"
        className="absolute hidden flex-col gap-2 items-center top-0 right-0 mt-10 bg-white h-auto w-screen mr-[-16px] lg:w-96 lg:mr-0 shadow-xl rounded-xl lg:rounded-tr-none"
      >
              ...
              cart.map((_v, _i) => {
                ...
                return (
                  ...
                  >
                    <Link
                      onClick={hideCartBox}
                      to={`/category/${_v?.category?.split("'")[0]}/products/${
                        _v?.id
                      }`}
