...
const Category = ({ dataFromServer }) => {
  const [data, setData] = useState(
    checkData(dataFromServer['firstData']) ? dataFromServer['firstData'] : []
  )
  const [error, setError] = useState(false)
  const [loading, setLoading] = useState(
    checkData(dataFromServer['firstData']) ? false : true
  )
  ...
  useEffect(() => {
    ...
    if (!checkData(dataFromServer['firstData'])) {
      ;(async () => {
        ...
        if (response instanceof Error) setError(logger(response))
        else {
          if (checkData(response)) setData(response)
          else setData([])
        }
        setLoading(false)
      })()
  ...
  const addToCart = (_v) => {
    setCart([
      ...cart,
      {
        ...
        price: _v?.price,
        image: _v?.image,
        category: Lowercase(_v?.category),
      },
    ])
  ...
  if (!loading && !error && !checkData(data)) return <NotFound />
  if (!loading && error !== false) return <p>{error}</p>
  return (
    <>
      <MetaTags
        ...
        description={
          data?.description || `This is home page of ${Capitalize(name)}`
        }
      ...
      {!loading && !error && checkData(data) && (
        <div className="flex flex-col justify-start items-start py-3">
          <div className="flex w-full justify-between items-center">
            <Link
              to={`/category/${Lowercase(name)}`}
              className={`text-xl w-auto hover:text-neutral-600 ${bgColor} p-4 rounded-t-lg`}
            >
              {Capitalize(postFixer(name))}
            </Link>
            <Link
              to={`/`}
              className={`text-lg flex items-center justify-between w-auto hover:text-neutral-600 `}
            >
              <span> Home Page </span>
              <svg
          ...
          <div
            className={`w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 justify-center items-center rounded-lg rounded-tl-none p-2 lg:p-6 from-slate-200 ${viaColor} to-slate-200 from-10% via-35% to-100%  xl:from-20% xl:via-50% xl:to-80% bg-gradient-to-bl`}
          >
            {checkData(data) &&
              data?.map((_v, _i) => {
                return (
                  <div
                    to={`/products/${_v.id}`}
                    key={`p_${_i}`}
                    className="flex flex-col gap-4 items-center p-4 border-2 bg-white hover:scale-95 transition-all duration-500 rounded-3xl sm:h-96"
                  >
                    <Link
                      to={`/category/${Lowercase(name)}/products/${_v?.id}`}
                      className="h-36 w-36 flex justify-center mt-4"
                    >
                      <img
                        width={160}
                        height={160}
                    ...
                    <div className={`${bgColor} p-2 sm:my-auto`}>
                      {_v?.price}$
                    </div>
                    <div className="flex items-center gap-2">
                      <Link
                        to={`/category/${name}/products/${_v?.id}`}
                        className="p-2 bg-slate-200 text-slate-600 hover:bg-opacity-80 rounded"
                      >
                        Details
                      </Link>
                      <button
                        ...
                        className="p-2 bg-rose-600 hover:bg-blue-400 text-white rounded select-none"
                      >
                        Add to cart
                      </button>
                    </div>
...
function postFixer(str) {
  if (str === 'men') return "Men's Clothing"
  else if (str === 'women') return "Women's Clothing"
  else return str
}
