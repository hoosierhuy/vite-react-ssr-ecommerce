...
const Post = ({ dataFromServer }) => {
  ...
  if (
    ...
    !error &&
    checkData(data) &&
    data?.category !== Lowercase(postFixer(name))
  )
    return <NotFound />
  ...
  return (
    <>
      <MetaTags
        ...
        description={
          data?.description || 'This is home page of my shopping website'
        }
      ...
      {!loading && !error && checkData(data) && (
        <div className="flex flex-col justify-start items-start py-0 ">
          <div className="flex w-full justify-between items-center py-5">
            <nav className="px-4">
              <ul className="flex gap-2 items-center">
                ...
                <li>
                  <Link className="p-1 rounded hover:bg-neutral-100" to={`/`}>
                    Home
                  </Link>
                </li>
                ...
                <li className="hidden lg:block text-neutral-600">
                  {data?.title}
                </li>
              </ul>
            </nav>
            <Link
              to={`/`}
              className={`hidden lg:flex items-center justify-between w-auto hover:text-neutral-600 `}
            >
              <span> Home Page </span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
          ...
          <div
            className={`w-full flex justify-center items-center rounded-2xl p-2 lg:p-6 from-slate-200 ${viaColor} to-slate-200 from-10% via-35% to-100%  xl:from-20% xl:via-50% xl:to-80% bg-gradient-to-bl`}
          >
            <div className="p-4 border-2 bg-white rounded-3xl w-full flex flex-col md:flex-row gap-4 justify-start">
              <div
                key={`post${data?.id}`}
                className="flex md:min-w-72 flex-col gap-4 items-center justify-center"
              >
                <div className="max-h-48 max-w-48 sm:max-h-64 sm:max-w-64 lg:max-h-96 lg:max-w-96 flex justify-center p-0 md:p-4">
                  <img
                    ...
                    alt={data?.title}
                    src={data?.image}
                    className="w-full  object-contain bg-center bg-no-repeat rounded-sm"
                  />
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="p-2 bg-rose-600 hover:bg-blue-400 text-white rounded text-nowrap select-none"
                  >
                    Add to cart
                  </button>
                </div>
