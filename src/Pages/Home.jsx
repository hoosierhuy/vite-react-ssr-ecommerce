...
const Home = ({ dataFromServer }) => {
  const data = checkData(dataFromServer['firstData'])
  ...
  const category1 =
    data?.length > 0
      ? data?.filter((itm) => itm.category === 'electronics')
      : []
  const category2 =
    data?.length > 0 ? data?.filter((itm) => itm.category === 'jewelery') : []
  const category3 =
    data?.length > 0
      ? data?.filter((itm) => itm.category === "men's clothing")
      : []
  const category4 =
    data?.length > 0
      ? data?.filter((itm) => itm.category === "women's clothing")
      : []
  ...
  return (
    <>
      ...
      <div
        key="main"
        className="flex flex-col gap-5 w-full justify-between py-3"
      >
