...
export const checkData = (obj) => {
  try {
    ...
    else if (typeof obj === 'object' && Object.keys(obj).length) return true
    return false
  } catch (error) {
    return false
  }
  ...
  try {
    ...
    if (err instanceof Error) {
      ...
      if (projectDevelopMode) {
        ...
        return (
          <>
            <span className="text-lg font-bold block my-5">
              Error details: <a>(in developer mode)</a>
            </span>
            <span className="text-wrap">
              {' '}
              <b className="text-sky-800">Code:</b> {errorMsg?.code}
            </span>
            <br />
            <span className="text-wrap">
              {' '}
              <b className="text-sky-800">Message:</b> {errorMsg?.message}
            </span>
            <br />
            <span className="text-wrap">
              {' '}
              <b className="text-sky-800">Name:</b> {errorMsg?.name}
            </span>
            <br />
            <span className="text-wrap">
              {' '}
              <b className="text-sky-800">Status:</b> {errorMsg?.status}
            </span>
            <br />
            <span className="text-wrap break-words inline-block max-w-full">
              {' '}
              <b className="text-sky-800">Stack:</b> {errorMsg?.stack}
            </span>
            <span className="text-wrap break-words inline-block max-w-full">
              {' '}
              <b className="text-sky-800">Config:</b>{' '}
              {JSON.stringify(errorMsg?.config)}
            </span>
          </>
        )
      }
      //In production mode:
        ...
        if (err.response)
          if (err.response.status === 404) return '404 API Data Not Found!'
